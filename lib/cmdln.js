...
Cmdln.prototype.bashCompletionSpec = function bashCompletionSpec(opts) {
    ...
    var aliases = [];
    var allAliases = [];
    var aliasesDescription= [];
    var allAliasesDescription = [];
    Object.keys(this._nameFromAlias).sort().forEach(function (alias) {
        ...
        if (includeHidden || !handler.hidden) {
            aliases.push(alias);
            desc = ((handler.desc || handler.help.split('\n')[0]))
            aliasesDescription.push(dashdash.COMPLETION_DESCRIPTION(desc);
        }
        allAliases.push(alias);
        desc = ((handler.desc || handler.help.split('\n')[0]))
        allAliasesDescription.push(dashdash.COMPLETION_DESCRIPTION(desc));
    });
    ...
    spec.push(format('local cmd%s_subcmdsdescription="%s"', context, aliasesDescription.join('|')));
    spec.push(format('local cmd%s_allsubcmdsdescription="%s"', context, allAliasesDescription.join('|')));
